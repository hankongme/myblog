...
(function ($) {
    ...
    /* 基础对象检测 */
    DCNETPHP || $.error("DCNETPHP基础配置没有正确加载！");
     ...
     */
    DCNETPHP.parse_url = function (url) {
    ...
    DCNETPHP.parse_str = function (str) {
    ...
    DCNETPHP.parse_name = function (name, type) {
        if (type) {
            /* 下划线转驼峰 */
    ...
    DCNETPHP.U = function (url, vars, suffix) {
        ...
        if (0 === url.indexOf("/")) { //路由模式
            ...
            path[1] || $.error("DCNETPHP.U(" + url + ")没有指定控制器");
    ...
    /* 设置表单的值 */
    DCNETPHP.setValue = function (name, value) {
        ...
        } else if (input[0]&&input[0].nodeName == "SELECT") {
            //增加select选中兼容情况

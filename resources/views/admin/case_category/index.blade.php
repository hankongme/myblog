            ...
            <div class="panel-heading">
                ...
                <div class="panel-actions">
                    ...
                    <button type="button" class="btn btn-primary"
                            onclick="edit('添加','{{url(__ADMIN_PATH__.'/case_category/create')}}')">
                    ...
                    @if($data['data'])
                        @foreach($data['data'] as $v)
                            <tr>
                                ...
                                <td>{{$v['id']}}</td>
                                <td>
                                    <a href="{{url(__ADMIN_PATH__.'/case_category/index',[$v['id']])}}">
                                        {{$v['name']}}
                                    </a>
                                ...
                                <td>
                                    ...
                                    @can('admin.case_category.edit')
                                    <a class="btn btn-xs btn-white"
                                       onclick="edit('编辑','{{url(__ADMIN_PATH__.'/case_category',['edit','id',$v['id']])}}')"
                                       data-toggle="tooltip" data-placement="left" title="编辑"><i
                                                class="fa fa-edit"></i></a>
                                    <a class="ajax-get btn btn-xs btn-white"
                                       href="{!! url(__ADMIN_PATH__.'/case_category/change',['key','status','id',$v['status'],'value',$v['status']?0:1]) !!}"
                                       data-toggle="tooltip" data-placement="left"
                                       title="{{$v['status']?'隐藏':'显示'}}"><i
                                                class="fa fa-{{$v['status']?'stop':'play-circle'}}"></i></a>
                                    @endcan

                                    @can('admin.case_category.del')
                                    <a class="confirm ajax-get btn btn-xs btn-white"
                                       href="{!! url(__ADMIN_PATH__.'/case_category/del',['id',$v['id']]) !!}"
                                       data-toggle="tooltip" data-placement="left"
                                       title="删除">
